...
export function isTabMasterInstalled(): boolean {
  try {
    ...
    return plugins.some(
      (p: any) => p.name === "TabMaster" || p.name === "Tab Master",
    );
  } catch {
    return false;
  }
...
export class UnifideckTabContainer {
  ...
  constructor(tab: UnifideckTab) {
    ...
    this.collection = {
      ...
      GetAppCountWithToolsFilter: (appFilter: any) =>
        this.collection.visibleApps.filter((app: SteamAppOverview) =>
          appFilter.Matches(app),
        ).length,
  ...
  buildCollection() {
    try {
      ...
      const allGamesCollection = (window as any).collectionStore?.GetCollection(
        "type-games",
      );
      ...
      console.log(
        `[Unifideck] Tab "${this.title}" has ${filteredApps.length} games`,
      );
    } catch (error) {
...
class TabManager {
  ...
  async initialize() {
    ...
    console.log(
      "[Unifideck] TabManager initialized with",
      this.tabs.length,
      "tabs",
    );
  }
  ...
  async loadGameCache(): Promise<void> {
    ...
    try {
      ...
      if (Array.isArray(games) && games.length > 0) {
        ...
        this.epicGameCount = games.filter(
          (g: any) => g.store === "epic",
        ).length;
      ...
      try {
        ...
        const validShortcuts = await call<[], number[]>(
          "get_valid_third_party_shortcuts",
        );
