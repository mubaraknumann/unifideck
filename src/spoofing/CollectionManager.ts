...
function getAllApps(): AppStoreOverview[] {
  ...
  try {
    ...
    console.log(
      `[Unifideck Collections] Found ${apps.length} apps in type-games`,
    );
    return apps;
  } catch (e) {
...
async function deleteCollection(collection: Collection): Promise<void> {
  try {
    ...
    console.log(
      `[Unifideck Collections] Deleted stale collection: ${collection.displayName}`,
    );
  } catch (e) {
    console.error(
      `[Unifideck Collections] Failed to delete ${collection.displayName}:`,
      e,
    );
  }
}
...
async function cleanupStaleCollections(): Promise<void> {
  ...
  try {
    ...
    console.log(
      "[Unifideck Collections] Error accessing userCollections, skipping cleanup:",
      e,
    );
    return;
  }
  ...
  if (!userCollections || !Array.isArray(userCollections)) {
    console.log(
      "[Unifideck Collections] userCollections not available, skipping cleanup",
    );
    return;
  }
  ...
  console.log(
    `[Unifideck Collections] Checking ${userCollections.length} user collections for stale entries...`,
  );
...
async function getOrCreateCollection(tag: string): Promise<Collection | null> {
  ...
  if (!collection) {
    console.error(
      `[Unifideck Collections] Failed to create collection: ${tag}`,
    );
    return null;
  }
...
async function syncTabToCollection(
  ...
  console.log(
    `[Unifideck Collections] "${collectionName}": ${overviews.length} apps`,
  );
  return true;
}
...
export async function syncUnifideckCollections(): Promise<void> {
  ...
  console.log(
    `[Unifideck Collections] Syncing ${UNIFIDECK_TABS.length} tabs to collections`,
  );
  ...
  console.log(
    `[Unifideck Collections] ✓ Sync complete: ${succeeded}/${UNIFIDECK_TABS.length} collections updated`,
  );
}
