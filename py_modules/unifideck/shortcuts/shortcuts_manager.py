"""
Shortcuts Manager - Manages Steam's shortcuts.vdf file for non-Steam games.

Extracted from main.py to reduce monolith size.
Handles shortcut CRUD, deduplication, registry, games.map cache, and reconciliation.
"""
...
# Try to import Game from backend
try:
    from py_modules.unifideck.stores.base import Game
except ImportError:
    Game = None  # Will be passed as parameter if not importable
...
class ShortcutsManager:
    ...
    def __init__(self, steam_path: Optional[str] = None, plugin_dir: Optional[str] = None):
        self.steam_path = steam_path or self._find_steam_path()
        self.plugin_dir = plugin_dir or os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    ...
    def repair_shortcuts_exe_path(self) -> Dict[str, Any]:
        ...
        current_launcher = os.path.join(self.plugin_dir, 'bin', 'unifideck-launcher')
    ...
    async def read_shortcuts(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Passive dedup: cull any duplicates Steam may have introduced
            deduped = self._deduplicate_shortcuts_data(data)
            if deduped > 0:
                logger.info(f"[Dedup] Removed {deduped} duplicate(s) on read")
                save_shortcuts_vdf(self.shortcuts_path, data)
    ...
    def _deduplicate_shortcuts_data(self, shortcuts_data: Dict[str, Any]) -> int:
        """Remove duplicate Unifideck shortcuts from shortcuts data in-place.
        
        When Steam's client rewrites shortcuts.vdf from its in-memory state,
        it can create duplicate entries for the same game. This method detects
        and removes those duplicates, keeping the entry with the most accumulated
        Steam metadata (play history, artwork, playtime).
        
        Args:
            shortcuts_data: The shortcuts dict (modified in-place)
        ...
        # Group Unifideck shortcuts by canonical LaunchOptions
        groups = defaultdict(list)
        for idx, shortcut in shortcuts.items():
            launch = shortcut.get('LaunchOptions', '')
            full_id = get_full_id(launch)
            if full_id:  # Only Unifideck shortcuts have store:id patterns
                groups[full_id].append(idx)
        
        # Find and remove duplicates
        to_remove = []
        for full_id, indices in groups.items():
            if len(indices) <= 1:
                continue
            
            # Score each candidate — keep the "richest" one (most Steam metadata)
            # The ORIGINAL shortcut accumulates Steam metadata over time:
            #   - LastPlayTime (non-zero = has been played)
            #   - icon (non-empty = has artwork set)
            #   - Playtime_Forever / other Steam-injected fields
            # The DUPLICATE typically has none of these.
            def score(idx):
                s = shortcuts[idx]
                pts = 0
            ...
            for idx in indices:
                if idx != best:
                    logger.info(f"[Dedup] Removing duplicate for {full_id}: "
                               f"keeping idx={best} (score={score(best)}), "
                               f"removing idx={idx} (score={score(idx)})")
    ...
    async def deduplicate_shortcuts(self) -> int:
        """Read shortcuts.vdf from disk, remove duplicates, and write back.
        
        Call this before sync operations to ensure Steam-introduced duplicates
        are cleaned up before we make further changes.
        ...
        try:
            data = load_shortcuts_vdf(self.shortcuts_path)
            removed = self._deduplicate_shortcuts_data(data)
            
            if removed > 0:
                save_shortcuts_vdf(self.shortcuts_path, data)
                # Invalidate cache so next read picks up deduped data
                self._shortcuts_cache = None
                logger.info(f"[Dedup] Pre-sync: removed {removed} duplicate shortcut(s)")
                        ...
                        if app_id and app_id in appid_to_launch_opts:
                            ...
                            if game:
                                ...
                                if game.is_installed:
                                    if game.store == 'epic':
                                        metadata = await epic_client.get_installed() if epic_client else {}
                                        ...
                                        game_info = gog_client.get_installed_game_info(game.id) if gog_client else None
                                        if game_info and game_info.get('executable'):
                                            exe_path = game_info['executable']
                                            work_dir = os.path.dirname(exe_path)
                                            await self._update_game_map(game.store, game.id, exe_path, work_dir)
                                    elif game.store == 'amazon':
                                        game_info = amazon_client.get_installed_game_info(game.id) if amazon_client else None
                                        if game_info and game_info.get('executable'):
                                            exe_path = game_info['executable']
                                            work_dir = os.path.dirname(exe_path)
                                            await self._update_game_map(game.store, game.id, exe_path, work_dir)
