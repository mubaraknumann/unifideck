...
from backend.utils.steam_user import get_logged_in_steam_user, migrate_user0_to_logged_in_user

# Import SteamGridDB client
try:
    from backend.artwork import SteamGridDBClient
...
from backend.saves import CloudSaveManager

# Import resilient launch options parser
from backend.utils.shortcuts import extract_store_id, is_unifideck_shortcut, get_full_id, get_store_prefix
...
from backend.services import InstallService, ArtworkService, MetadataService, SyncService
...
class Plugin:
    ...
    async def _main(self):
        ...
        logger.info("[INIT] Initializing SyncService")
        self.sync_service = SyncService(
            ...
            artwork_service=self.artwork_service,
            size_fetcher=self.size_fetcher,
            sync_progress=self.sync_progress,
            plugin_dir=DECKY_PLUGIN_DIR
        )

        # Global lock to prevent concurrent syncs (DEPRECATED - moved to SyncService)
        # Keep these for backwards compatibility with any direct references
    ...
    async def sync_libraries(self, fetch_artwork: bool = True) -> Dict[str, Any]:
        ...
        Delegates to SyncService for the actual implementation.
        """
        # Update legacy flags for backwards compatibility
        self._is_syncing = self.sync_service._is_syncing
        self._cancel_sync = self.sync_service._cancel_sync
        
        result = await self.sync_service.sync_libraries(fetch_artwork=fetch_artwork)
    ...
    async def force_sync_libraries(self, resync_artwork: bool = False) -> Dict[str, Any]:
        ...
        result = await self.sync_service.force_sync_libraries(resync_artwork=resync_artwork)
    ...
    async def cancel_sync(self) -> Dict[str, Any]:
        ...
        # Sync flags for backwards compatibility
        self._is_syncing = self.sync_service._is_syncing
        self._cancel_sync = self.sync_service._cancel_sync
